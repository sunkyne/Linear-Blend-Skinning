...
map< int, vector<glm::mat4> > animation;
vector<glm::mat4> bind;
vector<glm::mat4> invBind;
int framecount = 0;
int bones = 0;
...
void render()
{
	...
	// Draw character
	double fps = 30;
	int frameCount = framecount; // TODO: This number needs to be modified
	...
	for(const auto &shape : shapes) {
		...
		// Draw bone
		// TODO: implement
		if (keyToggles[(unsigned int)'z']) {
			progSimple->bind();
			glUniformMatrix4fv(progSimple->getUniform("P"), 1, GL_FALSE, glm::value_ptr(P->topMatrix()));
			for (int i = 0; i < bones; i++) {
				MV->pushMatrix();
				if (t != 0) {	
					MV->multMatrix(animation[frame][i]);
				}
				else {
					MV->multMatrix(bind[i]);
				}
				glUniformMatrix4fv(progSimple->getUniform("MV"), 1, GL_FALSE, glm::value_ptr(MV->topMatrix()));
				glLineWidth(3);
				glBegin(GL_LINES);
		...
		if (t != 0) {
			shape->update(frame, animation, invBind);
		}
		shape->draw(frame);
		progSkin->unbind();
...
void loadSkeletonFile() {
	...
	for (int j = 0; j < bones; j++) {
		...
		bind.push_back(M);
		// inverse bind
		invBind.push_back(glm::inverse(M));
	}
