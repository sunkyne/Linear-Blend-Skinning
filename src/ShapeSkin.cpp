	...
	for (int i = 0; i < numVerts; i++) {
		...
		for (int i = 0; i < maxInf; i++) {
			if (i < numInf) {
				float ind, weight;
				ss >> ind >> weight;
				indBuf.push_back(ind);
				weightBuf.push_back(weight);
			}
			else {
				indBuf.push_back(0.0f);
...
void ShapeSkin::update(int k, map< int, vector<glm::mat4> > animation, vector<glm::mat4> bind)
{
	// TODO: CPU skinning calculations.
	vector<glm::mat4> anim = animation[k];

	vector<glm::mat4> boneProd;
	for (int j = 0; j < bones; j++) {
		boneProd.push_back(anim[j] * bind[j]);
	}
	for (int i = 0; i < numVerts; i++) {
		int ind = i * maxInf;
		int posInd = i * 3;
		glm::vec4 initPos(initPosBuf[posInd], initPosBuf[posInd + 1], initPosBuf[posInd + 2], 1.0f);
		glm::vec4 initNorm(initNorBuf[posInd], initNorBuf[posInd + 1], initNorBuf[posInd + 2], 0.0f);
		glm::vec4 sumPos(0.0f);
		glm::vec4 sumNorm(0.0f);
		for (int x = 0; x < maxInf; x++) {
			int boneInd = indBuf[ind + x];
			if (boneInd == 0 && x != 0) {
				break;
			}
			sumPos += weightBuf[ind + x] * boneProd[boneInd] * initPos;
			sumNorm += weightBuf[ind + x] * boneProd[boneInd] * initNorm;
		}
		posBuf[posInd] = sumPos.x;
		posBuf[posInd + 1] = sumPos.y;
		posBuf[posInd + 2] = sumPos.z;
		norBuf[posInd] = sumNorm.x;
		norBuf[posInd + 1] = sumNorm.y;
		norBuf[posInd + 2] = sumNorm.z;
	}
